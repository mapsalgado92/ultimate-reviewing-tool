...
export default function Tool() {
  ...
  const pull_handler = async () => {
    ...
    const pull_origin = objectify(
      await sheets.pull_data(
        `reviews-pull-${lob.toLowerCase()}`,
        auth.authorization()
      )
    ).filter((o) => o.rated_date > limit_date)
    const updated = objectify(
      await sheets.pull_data("reviews-updated", auth.authorization())
    ).reverse()

    const rca = await sheets.pull_rca(
      `rca-${lob.toLowerCase()}-json`,
      auth.authorization()
    )
    ...
    console.log("MERGED", merged, "RCA", rca[0])
    ...
    <main>
      ...
      >
        <div className="column is-3 is-flex is-flex-direction-column mb-0 card is-fullheight">
          <div className="tag">
            {auth.logged ? auth.user.username : "Not logged in..."}
          </div>
          {auth.logged ? (
            <button
            ...
            <Link href="/">
              <div className="button is-danger is-light is-fullwidth">
                GO TO LOGIN PAGE
              </div>
            </Link>
          )}
